...
implements TabExecutor {
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            return this.filter(java.util.Arrays.asList("status", "on", "off", "reload", "check", "exempt", "unexempt", "violations", "clearvl"), args[0]);
        }
        if (args.length == 2 && java.util.Arrays.asList("check", "exempt", "unexempt", "violations", "clearvl").contains(args[0].toLowerCase(Locale.ROOT))) {
            return this.filter(Bukkit.getOnlinePlayers().stream().map(Player::getName).collect(java.util.stream.Collectors.toList()), args[1]);
        }
        if (args.length == 3 && args[0].equalsIgnoreCase("exempt")) {
            return this.filter(java.util.Arrays.asList("5", "10", "15", "30", "60"), args[2]);
        }
