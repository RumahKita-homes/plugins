...
public final class ViolationTracker {
    ...
    public int add(UUID uuid, String type) {
        Map<String, Integer> playerMap = this.violations.computeIfAbsent(uuid, k -> new HashMap<>());
    ...
    public int get(UUID uuid, String type) {
        return this.violations.getOrDefault(uuid, java.util.Collections.emptyMap()).getOrDefault(type, 0);
    }
